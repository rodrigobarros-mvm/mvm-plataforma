...
import { AlertTriangle, Clock, TrendingUp, XCircle, CheckCircle2, Zap } from "lucide-react";
...
export default function PipelineHealth() {
  ...
  const paradas7 = analise.filter((o: any) => o.diasParado >= 7 && o.status !== "ganho" && o.status !== "perdido" && o.status !== "cancelado");
  ...
  const perdidos = analise.filter((o: any) => o.status === "perdido");
    ...
    <Card className="border-border">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="text-center p-3 rounded-xl border border-border">
            <p className="text-2xl font-black" style={{ color: alertas.length > 0 ? "#e21d3c" : "#059669" }}>{alertas.length}</p>
            <p className="text-xs text-muted-foreground">Alertas</p>
          </div>
          <div className="text-center p-3 rounded-xl border border-border">
            <p className="text-2xl font-black" style={{ color: conversao >= 20 ? "#059669" : "#D97706" }}>{conversao}%</p>
            ...
            {alertas.map((a, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate">{a.opp.leadNome ?? a.opp.leadRazao ?? `Opp #${a.opp.id}`}</p>
                  <p className="text-xs" style={{ color: a.color }}>{a.msg}</p>
                </div>
                <Badge style={{ background: a.color + "20", color: a.color, fontSize: "10px" }}>{a.opp.modeloInteresse ?? "—"}</Badge>
              </button>
            ))}
          </div>
        )}
